...
bool dp_asus_validate_dp_version(struct dp_panel *dp_panel)
{
	...
	// some of ASUS monitors try to report DP1.4 but they are actually 1.2,
	// to avoid training failure due to select the wrong training pattern
	// AUS: PA329C, XG49VQ, PG27UQ
	if (!strncmp(dp_panel->asus_vendor, "AUS", 3) &&
			(dp_panel->asus_proc_codes == 0x49a ||
			 dp_panel->asus_proc_codes == 0x326 ||
			 dp_panel->asus_proc_codes == 0x27a))
		return true;
	// ACI: MG279Q
	if (!strncmp(dp_panel->asus_vendor, "ACI", 3) &&
			(dp_panel->asus_proc_codes == 0x27a))
