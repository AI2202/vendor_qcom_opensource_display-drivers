...
static void get_reg_cmd(char cmd, int len, int head)
{
	char tmp[256];
	int i = 0, rc = 0, start = 0;
	u8 *tx_buf, *return_buf, *status_buf;
	...
	struct dsi_cmd_desc cmds;
	struct mipi_dsi_msg tcon_cmd = {0, 0x06, 0, 0, 0, sizeof(cmd), NULL, len, NULL};
	const struct mipi_dsi_host_ops *ops = g_display->panel->host->ops;
	...
	if (g_display->ctrl[0].ctrl->current_state.controller_state == DSI_CTRL_ENGINE_ON) {
		// buffer assigned
		tx_buf = &cmd;
		return_buf = kcalloc(len, sizeof(unsigned char), GFP_KERNEL);
		status_buf = kzalloc(SZ_4K, GFP_KERNEL);
		memset(status_buf, 0x0, SZ_4K);
		...
		cmds.msg.rx_buf = status_buf;
		cmds.msg.rx_len = len;
		memset(g_reg_read, 0, REG_BUF_SIZE*sizeof(char));

		rc = ops->transfer(g_display->panel->host, &cmds.msg);
		if (rc <= 0) {
			DSI_LOG("rx cmd transfer failed rc=%d\n", rc);
		} else {
			memcpy(return_buf + start, status_buf, len);
			start += len;

			for(i=head; i<len; i++) {
				memset(tmp, 0, 256*sizeof(char));
				snprintf(tmp, sizeof(tmp), "%02X", return_buf[i]);
				strcat(g_reg_read,tmp);
			}
		}
	} else {
		DSI_LOG("DSI_CTRL_ENGINE is off\n");
				...
				size_t count, loff_t *ppos)
{
	...
	if (strlen(g_reg_read) == 0) {
		get_reg_cmd(0xa1, 11, 4);
		DSI_LOG("get_reg_cmd = %s", g_reg_read);
	}

	len += sprintf(buff, "%s%s\n", g_reg_read, g_lcd_unique_id);
	ret = simple_read_from_buffer(buf, count, ppos, buff, len);
	kfree(buff);
