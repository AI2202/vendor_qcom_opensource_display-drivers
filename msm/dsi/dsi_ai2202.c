...
static void dsi_ai2202_aod_backlight(struct dsi_panel *panel)
{
	...
	if (panel->aod_state ) {
		if (panel->panel_last_backlight == 64 || panel->panel_last_backlight == 4) {
			if (panel->panel_last_backlight == 64 && panel->panel_last_backlight != panel->panel_aod_last_bl) {
				...
				panel->aod_mode = 2;
			} else if (panel->panel_last_backlight == 4 && panel->panel_last_backlight != panel->panel_aod_last_bl) {
...
void ai2202_set_dc_bl_process(struct drm_encoder *encoder, struct drm_crtc *crtc, bool before)
{
	int rc = 0;
	...
	if (g_display->panel->dc_mode && before) {
		//off->on: set bl after kick off
		return;
	} else if (!g_display->panel->dc_mode && !before){
		//on->off: set bl before kick off
		return;
	}	
